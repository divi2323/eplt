"use client";

import { useDeferredValue, useEffect, useLayoutEffect, useMemo, useState } from "react";

type League = {
  id: string;
  league_number: string;
  name: string;
  notes: string | null;
  is_archived: boolean;
  league_type: "rolling" | "season" | null;
  league_status: "active" | "inactive" | null;
  start_date: string | null;
  end_date: string | null;
  runner_id: string | null;
  created_at?: string;
  updated_at?: string;
};

type PlayerOption = { id: string; player_number: string; first_name: string; last_name: string; is_active: boolean; };

type FilterMode = "active" | "archived" | "all";

function rowSearchText(l: League): string {
  const parts: string[] = [];
  parts.push(String(l.league_number ?? ""));
  parts.push(String(l.name ?? ""));
  parts.push(String(l.notes ?? ""));
  parts.push(String(l.league_type ?? ""));
  parts.push(String(l.league_status ?? ""));
  parts.push(String(l.start_date ?? ""));
  parts.push(String(l.end_date ?? ""));
  parts.push(l.is_archived ? "archived" : "active");
  for (const v of Object.values(l as any)) {
    if (v == null) continue;
    const t = typeof v;
    if (t === "string" || t === "number" || t === "boolean") parts.push(String(v));
  }
  return parts.join(" ").toLowerCase();
}

function playerLabel(p: PlayerOption) {
  const n = `${p.last_name ?? ""}, ${p.first_name ?? ""}`.replace(/^,\s*/, "").trim();
  return `${p.player_number} — ${n}`;
}

export default function LeaguesCrud() {
  const [filter, setFilter] = useState<FilterMode>("active");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [items, setItems] = useState<League[]>([]);

  const [players, setPlayers] = useState<PlayerOption[]>([]);

  // Client-side fast filter
  const [query, setQuery] = useState("");
  const deferredQuery = useDeferredValue(query);

  const [open, setOpen] = useState(false);
  const [closing, setClosing] = useState(false);
  const [row, setRow] = useState<Partial<League> | null>(null);

  const [name, setName] = useState("");
  const [notes, setNotes] = useState("");
  const [leagueType, setLeagueType] = useState<"rolling" | "season" | "">("");
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [runnerId, setRunnerId] = useState<string>("");

  const OPEN_MS = 900;
  const CLOSE_MS = 520;

  async function load() {
    setLoading(true);
    setError(null);
    try {
      const [leaguesRes, playersRes] = await Promise.all([
        fetch(`/api/leagues?archived=all`, { cache: "no-store" }),
        fetch(`/api/players?status=all`, { cache: "no-store" }),
      ]);

      const leaguesData = await leaguesRes.json().catch(() => ({}));
      if (!leaguesRes.ok) throw new Error(leaguesData?.error || "Failed to load leagues");

      const playersData = await playersRes.json().catch(() => ({}));
      if (playersRes.ok) setPlayers(playersData?.players || []);

      setItems(leaguesData.leagues || []);
    } catch (e: any) {
      setError(e?.message || "Failed to load leagues");
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => {
    load();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const filteredItems = useMemo(() => {
    const q = deferredQuery.trim().toLowerCase();
    return (items || []).filter((r) => {
      if (filter === "active" && r.is_archived) return false;
      if (filter === "archived" && !r.is_archived) return false;
      if (!q) return true;
      return rowSearchText(r).includes(q);
    });
  }, [items, filter, deferredQuery]);

  const closeDrawer = () => {
    setClosing(true);
    window.setTimeout(() => {
      setOpen(false);
      setClosing(false);
      setRow(null);
    }, CLOSE_MS);
  };

  useLayoutEffect(() => {
    if (!open) return;
    const html = document.documentElement;
    const body = document.body;
    const prevHtmlOverflow = html.style.overflow;
    const prevBodyOverflow = body.style.overflow;
    html.style.overflow = "hidden";
    body.style.overflow = "hidden";
    body.classList.add("epltDrawerOpen");
    return () => {
      body.classList.remove("epltDrawerOpen");
      html.style.overflow = prevHtmlOverflow;
      body.style.overflow = prevBodyOverflow;
    };
  }, [open]);

  const openNew = () => {
    setRow({});
    setName("");
    setNotes("");
    setLeagueType("");
    setStartDate("");
    setEndDate("");
    setRunnerId("");
    setClosing(false);
    setOpen(true);
  };

  const openEdit = (r: League) => {
    setRow(r);
    setName(r.name || "");
    setNotes(r.notes || "");
    setLeagueType((r.league_type || "") as any);
    setStartDate(r.start_date || "");
    setEndDate(r.end_date || "");
    setRunnerId(r.runner_id || "");
    setClosing(false);
    setOpen(true);
  };

  async function save() {
    if (!name.trim()) {
      alert("League name is required.");
      return;
    }

    try {
      const isEdit = Boolean(row && (row as any).id);
      const url = isEdit ? `/api/leagues/${(row as any).id}` : "/api/leagues";
      const method = isEdit ? "PATCH" : "POST";

      const payload: any = {
        name: name.trim(),
        notes: notes.trim() ? notes.trim() : null,
        league_type: leagueType ? leagueType : null,
        start_date: startDate ? startDate : null,
        end_date: endDate ? endDate : null,
        runner_id: runnerId ? runnerId : null,
      };

      const res = await fetch(url, {
        method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || "Save failed");

      await load();
      closeDrawer();
    } catch (e: any) {
      alert(e?.message || "Save failed");
    }
  }

  async function archiveToggle() {
    if (!row?.id) return;
    const doArchive = !(row as any).is_archived;

    try {
      const res = await fetch(`/api/leagues/${row.id}/${doArchive ? "archive" : "unarchive"}`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || "Update failed");

      await load();
      const updated = (data.league || null) as League | null;
      if (updated) setRow(updated);
    } catch (e: any) {
      alert(e?.message || "Update failed");
    }
  }

  async function softDelete() {
    if (!row?.id) return;
    const ok = confirm("Soft delete this league? (It can be recovered later once we add a recycle bin.)");
    if (!ok) return;

    try {
      const res = await fetch(`/api/leagues/${row.id}`, { method: "DELETE" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data?.error || "Delete failed");

      await load();
      closeDrawer();
    } catch (e: any) {
      alert(e?.message || "Delete failed");
    }
  }

  return (
    <>
      <div
        style={{
          display: "grid",
          gridTemplateColumns: "78px max-content minmax(0, 1fr) max-content",
          width: "100%",
          minWidth: 0,
          gridTemplateRows: "auto auto",
          columnGap: 12,
          rowGap: 10,
          alignItems: "stretch",
        }}
      >
        <img
          src="/assets/poker_chip_diamond.png"
          width={78}
          height={78}
          alt="Leagues"
          style={{
            gridColumn: "1",
            gridRow: "1 / span 2",
            borderRadius: 999,
            filter: "drop-shadow(0 3px 10px rgba(0,0,0,0.65))",
            alignSelf: "center",
          }}
        />

        <div style={{ gridColumn: "2", gridRow: "1 / span 2", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <h1 style={{ margin: 0, fontSize: 44, lineHeight: 1.05 }}>Leagues</h1>
          <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 6 }}>Manage Leagues</div>
        </div>

        <div style={{ gridColumn: "3", gridRow: "1", position: "relative", alignSelf: "center", minWidth: 0 }}>
          <input
            className="input"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search…"
            style={{ width: "100%", paddingRight: 38 }}
          />
          {query ? (
            <button
              type="button"
              aria-label="Clear search"
              onClick={() => setQuery("")}
              className="btnGhost"
              style={{
                position: "absolute",
                right: 6,
                top: "50%",
                transform: "translateY(-50%)",
                padding: "6px 10px",
                lineHeight: 1,
              }}
            >
              ×
            </button>
          ) : null}
        </div>

        <button
          className="btn"
          onClick={openNew}
          style={{
            gridColumn: "4",
            gridRow: "1 / span 2",
            justifySelf: "end",
            height: "100%",
            minHeight: 78,
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
            paddingInline: 18,
          }}
        >
          <span style={{ display: "flex", flexDirection: "column", alignItems: "center", lineHeight: 1.05 }}>
            <span style={{ fontSize: 20, fontWeight: 900 }}>+</span>
            <span>League</span>
          </span>
        </button>

        <div style={{ gridColumn: "3", gridRow: "2", display: "flex", gap: 8, alignItems: "center", alignSelf: "center" }}>
          <button className={filter === "active" ? "btn" : "btnGhost"} onClick={() => setFilter("active")}>Active</button>
          <button className={filter === "archived" ? "btn" : "btnGhost"} onClick={() => setFilter("archived")}>Archived</button>
          <button className={filter === "all" ? "btn" : "btnGhost"} onClick={() => setFilter("all")}>All</button>
        </div>
      </div>

      {error && (
        <div className="panel" style={{ marginTop: 12, padding: 12, borderColor: "rgba(255,80,80,.25)" }}>
          <b style={{ color: "#ffd1d1" }}>Error:</b> {error}
          <div style={{ color: "var(--muted)", marginTop: 6 }}>
            If this is a schema error, run the latest DB migration in <span className="mono">/home/eplt/db/migrations</span>.
          </div>
        </div>
      )}

      <div className="panel" style={{ padding: 0, marginTop: 12 }}>
        <table className="table">
          <thead>
            <tr>
              <th style={{ width: 160 }}>League #</th>
              <th>League Name</th>
              <th style={{ width: 140 }}>Status</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={3} style={{ padding: 14, color: "var(--muted)" }}>Loading…</td></tr>
            ) : filteredItems.length === 0 ? (
              <tr><td colSpan={3} style={{ padding: 14, color: "var(--muted)" }}>No leagues found.</td></tr>
            ) : (
              filteredItems.map((r) => (
                <tr key={r.id} className="rowHover" onClick={() => openEdit(r)}>
                  <td className="mono">{String(r.league_number ?? "")}</td>
                  <td>{r.name}</td>
                  <td><span className="badge">{r.is_archived ? "Archived" : "Active"}</span></td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>

      {open && (
        <div className={`drawerWrap ${closing ? "drawerClosing" : "drawerOpening"}`}>
          <div className="drawerBackdrop" onClick={closeDrawer} />

          <div className="drawer" style={{ animationDuration: `${OPEN_MS}ms` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
              <div>
                <h2 style={{ margin: 0 }}>{(row as any)?.id ? "Edit League" : "New League"}</h2>
                {(row as any)?.id && (
                  <div style={{ color: "var(--muted)", fontSize: 12, marginTop: 4 }}>
                    <span className="mono">{String((row as any).league_number ?? "")}</span> • <span className="mono">{(row as any).id}</span>
                  </div>
                )}
              </div>

              <div style={{ display: "flex", gap: 8 }}>
                {(row as any)?.id && (
                  <button className="btnGhost" onClick={archiveToggle}>{(row as any).is_archived ? "Unarchive" : "Archive"}</button>
                )}
                <button className="btnGhost" onClick={closeDrawer}>Close</button>
              </div>
            </div>

            <div className="hr" />

            <div style={{ display: "grid", gap: 12 }}>
              <div>
                <div className="fieldLabel">League Name</div>
                <input className="input" value={name} onChange={(e) => setName(e.target.value)} />
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="fieldLabel">Type</div>
                  <select className="input" value={leagueType} onChange={(e) => setLeagueType(e.target.value as any)}>
                    <option value="">(unset)</option>
                    <option value="rolling">Rolling</option>
                    <option value="season">Season</option>
                  </select>
                </div>
                <div>
                  <div className="fieldLabel">Runner</div>
                  <select className="input" value={runnerId} onChange={(e) => setRunnerId(e.target.value)}>
                    <option value="">(none)</option>
                    {players
                      .slice()
                      .sort((a, b) => playerLabel(a).localeCompare(playerLabel(b)))
                      .map((p) => (
                        <option key={p.id} value={p.id}>
                          {playerLabel(p)}{p.is_active ? "" : " (inactive)"}
                        </option>
                      ))}
                  </select>
                </div>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <div className="fieldLabel">Start Date</div>
                  <input className="input" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                </div>
                <div>
                  <div className="fieldLabel">End Date</div>
                  <input className="input" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                </div>
              </div>

              <div>
                <div className="fieldLabel">Notes</div>
                <textarea className="input" style={{ minHeight: 110 }} value={notes} onChange={(e) => setNotes(e.target.value)} />
              </div>

              <div style={{ display: "flex", gap: 10, justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ color: "var(--muted)", fontSize: 12 }}>
                  {(row as any)?.id ? "Changes save immediately to the database." : "A new league number will be assigned automatically."}
                </div>

                <div style={{ display: "flex", gap: 8 }}>
                  {(row as any)?.id && (
                    <button className="btnGhost" onClick={softDelete}>Delete</button>
                  )}
                  <button className="btn" onClick={save}>Save</button>
                </div>
              </div>
            </div>

          </div>
        </div>
      )}
    </>
  );
}
